...
let facialLoginInProgress = false; // ✅ NUEVO: Previene ejecuciones múltiples
...
// ==========================================
// 🔒 RECONOCIMIENTO FACIAL - VERSIÓN MEJORADA
// ==========================================

/**
 * ✅ VERSIÓN MEJORADA - Inicia sesión con reconocimiento facial
 * Previene ejecuciones múltiples y valida correctamente las capturas
 */
async function loginWithFace() {
  // ✅ PREVENIR EJECUCIONES MÚLTIPLES
  if (facialLoginInProgress) {
    console.warn('⚠️ Login facial ya en progreso, ignorando nueva ejecución');
    showAlert('Por favor espere, procesando reconocimiento facial...', 'info');
    return;
  }
  
  facialLoginInProgress = true;
  let stream = null;
  ...
  try {
    ...
    // ✅ VALIDACIÓN MEJORADA: Verificar que el video esté realmente activo
    if (videoElement.videoWidth === 0 || videoElement.videoHeight === 0) {
      throw new Error('El video no tiene dimensiones válidas');
    }
    
    if (videoElement.readyState < 2) {
      throw new Error('El video aún no está listo');
    }
    
    console.log('📐 Dimensiones del video:', videoElement.videoWidth, 'x', videoElement.videoHeight);
    console.log('📊 Estado del video (readyState):', videoElement.readyState);
    ...
    if (!stream || !stream.active) {
      throw new Error('La cámara se detuvo inesperadamente');
    }
    ...
    // ✅ VALIDACIÓN MEJORADA de la imagen capturada
    if (!imageBase64 || imageBase64.length < 1000) {
      throw new Error(`La imagen capturada es inválida (${imageBase64.length} bytes). Asegúrese de que la cámara esté funcionando correctamente.`);
    }
    
    // Detener la cámara ANTES de hacer la petición
    ...
    console.log('📊 Tamaño final a enviar:', imageBase64.length, 'caracteres');
    ...
    if (response.ok && result.status === 200) {
      ...
      // Login exitoso - Construir userData
      const userData = {
      ...
      console.warn('   1. El usuario no está registrado en el sistema');
      console.warn('   2. No tiene imagen de referencia en la base de datos');
      console.warn('   3. La imagen capturada no coincide con la registrada');
      
      showAlert(
        result.message || 
        'No se encontró coincidencia facial. Verifique que su rostro esté registrado en el sistema o intente con otro método de inicio de sesión.', 
        'warning'
      );
      facialLoginInProgress = false;
    } else {
    ...
    if (stream) {
      console.log('🛑 Deteniendo cámara por error...');
...
function handleFacialLoginError(result) {
  ...
  switch (result.status) {
    case 400:
      ...
      break;
    case 404:
      showAlert('No se encontró coincidencia facial. Verifique que esté registrado en el sistema o use otro método de login.', 'warning');
      break;
    case 500:
      showAlert('Error interno del servidor. Por favor intente más tarde o use otro método de login.', 'danger');
      break;
    default:
      showAlert(errorMessage, 'danger');
  }
